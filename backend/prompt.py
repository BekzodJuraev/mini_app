...
def crash_test(user_data):
    prompt = f"""
    Ты медицинский ИИ-эксперт. Твоя задача — оценить риски здоровья пользователя на основе его привычек. 

    ⚠️ Правила ответа:
    - Формат ответа должен быть СТРОГО в JSON (никакого текста вне JSON).
    - Используй ровно два ключа:
        "life_expectancy" (целое число лет, предполагаемая продолжительность жизни)
        "message" (строка с анализом и рекомендациями).
    - В "message" сначала укажи выявленный фактор риска (например, курение, алкоголь, стресс и т.д.).
    - Обязательно дай совет по снижению риска, при этом ВСЕГДА упоминай возможность использовать "Трекер привычек" в нашем веб-приложении как инструмент для контроля прогресса.
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {"role": "system",
             "content": "Ты эксперт в анализе здоровья и вредных привычек. Всегда отвечай только в формате JSON."},
            {"role": "user", "content": prompt}
        ],
        response_format={"type": "json_object"}
    )
