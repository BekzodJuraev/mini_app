...
def calories_edit(user_input: str, answer: str):

    """
    Редактирует существующий отчёт о калорийности по запросу пользователя.

    :param user_input: Текстовая команда от пользователя (например, "добавь яйцо 100 г")
    :param answer: Исходный ответ (в формате текста, например, message от AI)
    :return: Новый JSON-ответ с обновлённым составом блюда
    """

    prompt = f"""
Ты помощник по питанию. Пользователь дал тебе сгенерированный ранее отчёт о составе блюда и внёс правку.
Твоя задача — принять эту правку и обновить отчёт, строго следуя формату JSON ниже.

Примени изменение из пользовательского ввода к данному отчёту и верни результат.
...
Если еда не распознана:
{{
  "message": "Пожалуйста, предоставьте корректное описание еды для расчёта калорийности."
}}
...
"""
    ...
    messages = [
        {"role": "system", "content": "Ты — помощник по питанию. Всегда отвечай ТОЛЬКО в формате JSON."},
        {"role": "user", "content": prompt}
    ]
