...
def get_health_scale(height, weight, smoking_now, smoking_past, location, gender, date_birth, exp_smoke, smoke_what, smoke_day):
    ...
    Оцени состояние организма по шкале от 1 до 10.

    ВАЖНЫЕ ПРАВИЛА:
    - Все оценки должны быть реалистичными.
    - Курение должно значительно снижать показатели дыхательной, сердечно-сосудистой и иммунной систем.
    - Возраст влияет на общий уровень систем.
    - Вес и рост учитывай через ИМТ.
    - Не ставь всем системам одинаковые значения.
    - Используй дробные значения если нужно, например 6.4.

    ГЛАВНОЕ ПРАВИЛО:
    "Общий тонус" ОБЯЗАТЕЛЬНО рассчитывается как:
    (сумма всех "Общий показатель" систем + одиночных систем) / количество этих показателей.

    Нельзя придумывать "Общий тонус" отдельно.
    Он должен быть математически вычислен из остальных оценок.

    Внутри каждой системы сохрани все подпункты.
...
def environmental_and_data_risk_analysis_pet(data: dict) -> dict:
    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            ...
            response_format={"type": "json_object"},
            temperature=0.2  # Низкая температура для стабильности структуры JSON
        )
