...
def calories_edit(user_input: str, detail: list, current_detail: list):
    """
    Редактирует отчёт о калорийности.
    :param user_input: ТОЛЬКО для добавления новых блюд ("добавь яблоко")
    :param detail: Исходный список из базы (для контекста)
    :param current_detail: Актуальный список с фронта (уже измененный вручную)
    """

    prompt = f"""
Ты помощник по питанию. Твоя задача — сформировать итоговый отчет о КБЖУ.

У тебя есть два основных источника данных:
1. Текущий состав блюда (`current_detail`): это список, который пользователь уже отредактировал вручную (изменил вес или удалил что-то). Если в нем есть позиции только с названием и весом — рассчитай для них КБЖУ.
2. Новые позиции (`user_input`): если здесь указана еда, добавь её к текущему составу, рассчитав КБЖУ.

ВАЖНО: 
- Поле `user_input` используется ТОЛЬКО для добавления новых ингредиентов. 
- Не пытайся менять веса существующих позиций через `user_input`, бери их "как есть" из `current_detail`.

Ответ должен быть ТОЛЬКО в формате JSON:
{{
  "message": "Краткое описание действий (например: 'Добавил [еда] и обновил расчет')",
  "detail": [
    {{
      "еда": "название",
      "вес": 100,
...
Текущий состав (основа):
{json.dumps(current_detail, ensure_ascii=False)}

Добавить новые позиции из текста:
{user_input}
"""

    messages = [
        {"role": "system", "content": "Ты — помощник по питанию. Отвечай ТОЛЬКО в JSON. Твоя цель — дополнить текущий список новыми позициями из текста и рассчитать КБЖУ там, где их нет."},
        {"role": "user", "content": prompt}
    ]
