...
def breath_test(user_data):
    ...
    {{
      "message": "Вы задержали дыхание на {user_data['breathing_time']} секунд  это является [оценка реакции организма].хотелось бы видеть хотя бы ссылки на источники и рекоминлации"
    }}
...
def cooper_test(user_data):
    ...
    **Задание для AI:**  
    ...
    {{
      "message": "Исходя из полученных данных можем сделать вывод, что ваш уровень физической подготовки является средним.хотелось бы видеть хотя бы ссылки на источники и рекоминлации"
    }}
    """
