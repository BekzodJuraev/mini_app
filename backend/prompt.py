...
def chat_update(data, message):
    ...
    Твоя задача:
    1. Проанализируй сообщение и определи, к какой системе организма оно относится (например, "одышка" -> "Дыхательная система", "болит колено" -> "Опорно-двигательный аппарат").
    2. Если сообщение негативное — снизь показатели этой системы на 3-4 пункта.
    3. Если сообщение положительное (жалоб нет, стало лучше) — повысь на 2-3 пункта (макс. 10).
    4. Если сообщение нейтральное — ничего не меняй.
    5. Верни ВЕСЬ JSON целиком, обновив только затронутые части.
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {"role": "system",
             "content": "Ты медицинский анализатор. Ты обновляешь данные о здоровье пользователя на основе его текстовых жалоб или отчетов."},
            {"role": "user", "content": prompt}
        ],
        response_format={"type": "json_object"}
    )
