...
def blood_pressure_test(user_data):
    systolic = user_data["pressure_top"]
    diastolic = user_data["pressure_bottom"]
    ...
    - Без markdown.
    - Без списков.
    - Не изменяй значения давления.
    - Не изменяй категорию давления.
    - Внутри message обязательно укажи:
      1. текущее давление
      2. категорию давления
      3. краткие рекомендации по образу жизни
      4. упоминание, что однократное измерение не является диагнозом
      5. рекомендацию обратиться к врачу при стойких отклонениях

    Формат ответа:

    {{
      "message": "Ваше артериальное давление составляет {systolic}/{diastolic} мм рт. ст., что соответствует категории: {level}. Рекомендуется соблюдать режим сна, поддерживать физическую активность, контролировать питание и уровень стресса. Однократное измерение давления не является диагнозом и требует оценки в динамике. При стойких отклонениях рекомендуется обратиться к врачу для дополнительного обследования."
    }}
    """
    response = openai.ChatCompletion.create(
        model=MODEL,
        messages=[
            {"role": "system", "content": "Ты эксперт по физическим тестам. Проанализируй данные теста Давлениие."},
            {"role": "user", "content": prompt}
        ],
        response_format={"type": "json_object"}
    )
